...
KEYFILE_DIRECTORY = "/volume1/Pierre/dzgaboard/config"
...
class ReportState:
    ...
    def enable_report_state():
        filename = os.path.join(KEYFILE_DIRECTORY, "smart-home-key.json")
    ...
    def get_access_token(self):
        """Generates a signed JSON Web Token using a Google API Service Account."""
        signed_jwt = self.generate_jwt(os.path.join(KEYFILE_DIRECTORY, "smart-home-key.json"))
    ...
    def call_homegraph_api_key(payload):
        ...
        url = 'https://homegraph.googleapis.com/v1/devices:requestSync?key=' + config["general_settings"]["google_assistant"]["api_key"]
