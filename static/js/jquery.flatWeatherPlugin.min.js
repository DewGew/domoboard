...
;(function ( $, window, document, undefined ) {
		...
		var apiurls = {
			...
			"darksky" : ["https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/apikey/"],
		};
			...
			fetchWeather: function () {
				...
				for (var i = 0; i < apiurls[this.settings.api].length; i++) {
					...
					if (this.settings.api == "wunderground") {
						...
						//build wunderground url differently
						requests.push($.get(apiurls[this.settings.api][i]+ this.settings.latitude + "," + this.settings.longitude + "?units=auto&lang=" + this.settings.lang ));					
					}
					else {
		...
		*/
		function datamapper (input, settings) {
			...
			if (settings.api == "openweathermap") {
			    ...
			    out.today.sunrise = epochToHours(input.daily.data[0].sunriseTime, settings.timeformat);
			    out.today.sunset = epochToHours(input.daily.data[0].sunsetTime, settings.timeformat);
